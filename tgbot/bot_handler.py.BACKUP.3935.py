...
def send_random_text(bot, chat_id, people):
    ...
    people.save()
    reply_keyboard = [['%sمی‌خونم' % Emoji.MICROPHONE], ['⏩ برو جمله‌ی بعدی']]
...
def handle_text(bot, update):
    ...
    if record_obj.exists():
        ...
        elif 'نه دوباره می‌خوام بخونم' in text:
...
>>>>>>> 49ee567b5634c62dbccc10634a9e4afa91101b4a
    ...
    elif 'جمله‌ی بعدی' in text or 'برو جمله‌ی بعدی' in text:
        people.records.filter(state=RECORD_STATES_DEFAULT).delete()
        people.sentence_is_reading = None
        people.save()
        send_random_text(bot, chat_id, people)
